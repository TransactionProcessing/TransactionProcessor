...
public class MerchantDomainServiceTests {
    ...
    [Fact]
    public async Task MerchantDomainService_RecordDepositAgainstBalance_DepositIsMade()
    {
        this.AggregateService.Setup(m => m.Get<MerchantAggregate>(It.IsAny<Guid>(), It.IsAny<CancellationToken>()))
            .ReturnsAsync(Result.Success(TestData.Aggregates.CreatedMerchantAggregate()));
        this.AggregateService.Setup(m => m.GetLatest<MerchantBalanceAggregate>(It.IsAny<Guid>(), It.IsAny<CancellationToken>()))
            .ReturnsAsync(Result.Success(TestData.Aggregates.EmptyMerchantBalanceAggregate()));
        this.AggregateService
            .Setup(m => m.Save(It.IsAny<MerchantBalanceAggregate>(), It.IsAny<CancellationToken>()))
            .ReturnsAsync(Result.Success);

        Result result = await this.DomainService.RecordDepositAgainstBalance(TestData.Commands.RecordDepositCommand, CancellationToken.None);
        result.IsSuccess.ShouldBeTrue();
    }

    [Fact]
    public async Task MerchantDomainService_RecordDepositAgainstBalance_Failed_DepositIsNotMade()
    {
        this.AggregateService.Setup(m => m.Get<MerchantAggregate>(It.IsAny<Guid>(), It.IsAny<CancellationToken>()))
            .ReturnsAsync(Result.Success(TestData.Aggregates.CreatedMerchantAggregate()));
        this.AggregateService.Setup(m => m.GetLatest<MerchantBalanceAggregate>(It.IsAny<Guid>(), It.IsAny<CancellationToken>()))
    ...
    [Fact]
    public async Task MerchantDomainService_RecordWithdrawalAgainstBalance_WithdrawalIsMade()
    {
        this.AggregateService.Setup(m => m.Get<MerchantAggregate>(It.IsAny<Guid>(), It.IsAny<CancellationToken>()))
            .ReturnsAsync(Result.Success(TestData.Aggregates.CreatedMerchantAggregate()));
        this.AggregateService.Setup(m => m.GetLatest<MerchantBalanceAggregate>(It.IsAny<Guid>(), It.IsAny<CancellationToken>()))
        ...
        Result result = await this.DomainService.RecordWithdrawalAgainstBalance(TestData.Commands.RecordWithdrawalCommand, CancellationToken.None);
        result.IsSuccess.ShouldBeTrue();
    }

    [Fact]
    public async Task MerchantDomainService_RecordWithdrawalAgainstBalance_Failed_WithdrawalIsNotMade()
    {
        this.AggregateService.Setup(m => m.Get<MerchantAggregate>(It.IsAny<Guid>(), It.IsAny<CancellationToken>()))
            .ReturnsAsync(Result.Success(TestData.Aggregates.CreatedMerchantAggregate()));
        this.AggregateService.Setup(m => m.GetLatest<MerchantBalanceAggregate>(It.IsAny<Guid>(), It.IsAny<CancellationToken>()))
